...
import { isMuiElement, setRef } from '@material-ui/core/utils';
...
const SpeedDial = React.forwardRef(function SpeedDial(props, ref) {
  ...
  const [state, setState] = React.useState(initialNavigationState);
  ...
  if (!open && state.nextItemArrowKey !== undefined) {
  ...
  const handleKeyboardNavigation = event => {
    ...
    if (key === 'esc') {
      ...
      const nextAction = clamp(focusedAction + actionStep, 0, actions.current.length - 1);
      const nextActionRef = actions.current[nextAction];
      nextActionRef.focus();
      setState({ focusedAction: nextAction, nextItemArrowKey });
    }
    ...
    <div className={clsx(classes.root, actionsPlacementClass, classNameProp)} ref={ref} {...other}>
      ...
      >
        <Fab
          ...
          {...ButtonProps}
          ref={buttonRef => {
            actions.current[0] = buttonRef;
            setRef(origDialButtonRef, buttonRef);
          }}
        >
          {icon()}
        </Fab>
